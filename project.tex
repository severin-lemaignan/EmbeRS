...
WP3 will integrate together the modeling capabilities developed in WP2, with a
dual action policy: a policy driven by an artificial intrinsic motivation to act
socially (a social teleology), and a policy learned through human-in-the-loop
machine learning. This WP is high-risk/high-gain, as it proposes a very novel
approach to goal-driven socio-cognitive architectures, with the potential of
unlocking long-term social engagement~\cite{hoffman2019anki}. It is however a critical
component for the robot' deployments -- mitigations are however possible, by
decoupling the action policies if necessary.
...
interaction is very novel (existing literature on teleological robots focuses on
simple systems~\cite{oudeyer2005playground,forestier2017unified}). This task,
...
\textbf{T3.3 -- Integrating a socially-driven architecture for long-term
...
(WP4) capabilities of the robot, as well as the social policies created in T3.1 and
T3.2. T3.3 will specifically look at long term autonomy, including long-term
social goals, cognitive redundancy, and behavioural complexity.

T3.3 will also develop the arbitration mechanism that combines the robot's
social teleology (T3.1) with the human-taught action policy (T3.2). This
...
providing a comparison point for T4.2.

\textbf{T4.2 -- Generative neural network for social behaviour generation}
...
\begin{framed}
    ...
    \textbf{Timeframe:} \textbf{Y3-Y5}; one post-doc (shared with WP3).
...
the robot (R3), and implement them; in parallel to WP2, \textbf{WP3} transposes
the conceptual framework of WP1 into a principled cognitive architecture and
integrates together the cognitive functions of WP2 and WP4 (R2); \textbf{WP4}
...
\noindent\framebox[\linewidth]{Timeframe: \textbf{Y1-Y3}; one senior post-doc
with background in sociology of technology}
...
\noindent\framebox[\linewidth]{Timeframe: \textbf{Y1-Y4}; one post-doc in social
signal processing/machine learning/cognitive modelling}
...
\noindent\framebox[\linewidth]{Timeframe: \textbf{Y1-Y4}; one senior post-doc in
...
\noindent\framebox[\linewidth]{Timeframe: \textbf{Y2-Y5}; one post-doc in HRI/machine learning/learning from
demonstration}
...
\noindent\framebox[\linewidth]{Timeframe: \textbf{Y2-Y5}; one post-doc (shared
with WP3)}
...
guidelines for the robot's cognitive architecture (WP3), and will be addressed
there as well.
...
-   WP3: WP3 (the cognitive architecture) lays at the core of the project; the WP3 leader will be a senior 
    ...
    also oversee (with the PI) the experimental work taking place in WP5. A PhD student will also be recruited on 
    WP3, with a specific focus on the cognitive architecture design;

-   WP4: one post-doc (background in learning from demonstration and machine learning) will be in charge 
